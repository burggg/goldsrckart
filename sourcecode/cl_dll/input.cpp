...
void CL_AdjustAngles(float frametime, float *viewangles)
{
	...
	{
		V_StopPitchDrift();
		viewanglesReal[PITCH] -= speed*cl_pitchspeed->value * CL_KeyState(&in_forward);
		viewanglesReal[PITCH] += speed*cl_pitchspeed->value * CL_KeyState(&in_back);
	}

	up = CL_KeyState(&in_lookup);
	down = CL_KeyState(&in_lookdown);

	viewanglesReal[PITCH] -= speed*cl_pitchspeed->value * up;
	viewanglesReal[PITCH] += speed*cl_pitchspeed->value * down;
	...
	if (viewanglesReal[ROLL] > 50)
		viewanglesReal[ROLL] = 50;
	if (viewanglesReal[ROLL] < -50)
		viewanglesReal[ROLL] = -50;
}
...
void CL_DLLEXPORT CL_CreateMove(float frametime, struct usercmd_s *cmd, int active)
{
	...
	if (active && !Bench_Active())
	{
		...
		{
		camYaw[YAW] += cl_sidespeed->value * CL_KeyState(&in_right);
		camYaw[YAW] -= cl_sidespeed->value * CL_KeyState (&in_left);
		}
		*/

		//camYaw[YAW] += cl_sidespeed->value * CL_KeyState(&in_moveright);
		//camYaw[YAW] -= cl_sidespeed->value * CL_KeyState(&in_moveleft);
